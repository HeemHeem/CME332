...
int main(void){
    ...
    *(hex3_0_ptr) = (hex_display_cnvtr(hex2)<<16) + (hex_display_cnvtr(hex1)<<8)+ hex_display_cnvtr(hex0);
    ...
    while(1){
        ...
        //SNAPSHOT START
        *(interval_timer_ptr + 4) = 0x1;
        start = *(interval_timer_ptr+4)+ (*(interval_timer_ptr + 5) <<16);
       ...
       // get each digit starting with ones and then go up 
        ...
        //SNAPSHOT end
        *(interval_timer_ptr + 4) = 0x1;
        end = *(interval_timer_ptr+4)+ (*(interval_timer_ptr + 5) <<16);

        // ignore time restart values
        if ((start - end) > 0){
            int dummy1 = start - end;

            if (dummy1 > ex_max){
                printf("%d this is the new max execution time in clock cycles\n", dummy1);
                ex_max = dummy1;
            }

            if (dummy1 < ex_min){
                printf("%d this is the new min execution time in clock cycles\n", dummy1);
                ex_min = dummy1;
